...
TARGET_DIR="/var/www/egonzalezcortes-staging"
...
if [[ -z "${REMOTE}" ]]; then
  if [[ -z "${TARGET_HOST}" ]]; then
    ...
    echo "Or pass positional args: ./scripts/deploy-staging.sh egonzalezcortes@203.0.113.10"
    exit 1
  fi
...
echo "[2/5] Building ${APP_NAME} (staging mode)"
npm run --prefix "${REPO_DIR}" build:staging
